				...
				const struct i2c_device_id *i2c_id)
{
	...
	max8893->rdev = kzalloc(sizeof(struct regulator_dev *) * (pdata->num_subdevs + 1), GFP_KERNEL);
	if (!max8893->rdev) {
		kfree(max8893);
		return -ENOMEM;
	}
	...
	max8893->num_regulators = pdata->num_subdevs;
	for (i = 0; i < pdata->num_subdevs; i++) {

		DBG("regulator_register called for ldo=%d\n",pdata->subdevs[i].id);
		id = pdata->subdevs[i].id - MAX8893_LDO1;

		max8893->rdev[i] = regulator_register(&regulators_8893[id],
			max8893->dev, pdata->subdevs[i].initdata, max8893);
